...
- APK Signature Scheme v2で署名し、出力後に自己検証
...
`cordova.plugins.disusered.safe.encrypt/decrypt`互換のコールバックAPIも注入します。
...
```text
gradle -p template/android :app:assembleRelease
...
- 新規Google Playアプリの公開には通常AABが必要です。本版の出力はAPKです。
